  ...
  useEffect(() => {
    const fetchDrawerData = async () => {
      const value = await fetch(
        "https://api.github.com/repos/kunal-kushwaha/DSA-Bootcamp-Java/git/trees/main?recursive=1"
      )
        .then((res) => res.json())
        .then((val) => val["tree"])
        .then((data) =>
          data.filter((file) => file["path"].startsWith("assignments/"))
        );
      setDrawerData(value);
    };
    fetchDrawerData();
  ...
  return (
    <div>
      <div className={classes.appbar}>
        <button className={classes.drawerIcon} onClick={toggleDrawer}>
        ...
        {userCtx.isLoggedIn() ? (
          ...
          <button className={classes.actions} onClick={toggleModal}>
            Login
          </button>
        )}
      </div>
      <nav
        ...
        style={{ left: isDrawerOpen ? "0px" : "-30%" }}
      >
        <ul className={classes.drawerItems}>
          {drawerData.map((val, index) => (
            <li
              key={index}
              className={classes.drawerTile}
              onClick={() => {
                toggleDrawer();
